<!-- meta
title: 4.4 การแปลงเป็นทแยงมุมของเมทริกซ์สมมาตร
ch: 4
section: 4.4
page: ch4-4.html
-->
...
<h1 class="page-title">4.4 การแปลงเป็นทแยงมุมของเมทริกซ์สมมาตร</h1>
<p class="page-sub">ข่าวดีของเมทริกซ์สมมาตร (\(A^T = A\)) — มันแปลงเป็นทแยงมุมได้<em>เชิงตั้งฉาก</em>เสมอ:
มี \(P\) เชิงตั้งฉากปรกติที่ \(A = PDP^T\) — ทฤษฎีบทสเปกตรัมและการแยกเชิงสเปกตรัม พร้อมเทคนิคจับคู่กราม-ชมิดต์กับ eigen</p>
...
<section class="block" id="objectives">
  <div class="obj">
    ...
    <ul>
      <li>ตรวจว่าเมทริกซ์สมมาตรหรือไม่ และรู้สมบัติ: ค่าลักษณะเฉพาะเป็นจำนวนจริง, ปริภูมิลักษณะเฉพาะคนละค่าตั้งฉากกัน</li>
      <li>แปลงเมทริกซ์สมมาตรเป็นทแยงมุมเชิงตั้งฉาก: หา \(P\) เชิงตั้งฉากปรกติและ \(D\) ที่ \(A = PDP^T\)</li>
      <li>ใช้กราม-ชมิดต์ภายในปริภูมิลักษณะเฉพาะเมื่อค่าลักษณะเฉพาะซ้ำ</li>
      <li>เขียนการแยกเชิงสเปกตรัม \(A = \lambda_1\vec{u}_1\vec{u}_1^T + \cdots + \lambda_n\vec{u}_n\vec{u}_n^T\)</li>
    </ul>
  </div>
</section>
...
<section class="block" id="lesson">
  ...
  <h3>1) เมทริกซ์สมมาตร (symmetric matrix)</h3>
  <div class="box box-def">
    <div class="box-title">📐 นิยาม</div>
    <p>\(A\) เป็น<strong>สมมาตร</strong> เมื่อ \(A^T = A\) (สมาชิก \(a_{ij} = a_{ji}\) — สมมาตรรอบทแยง) เช่น \(\begin{bmatrix} 1 & -2\\ -2 & 1 \end{bmatrix}\)</p>
  </div>
  <div class="box box-thm">
    <div class="box-title">⭐ ทฤษฎีบท 4.4.1 — eigenspaces ตั้งฉากกัน</div>
    <p>ถ้า \(A\) สมมาตร แล้วปริภูมิลักษณะเฉพาะที่สมนัยกับค่าลักษณะเฉพาะ<em>ต่างกัน</em>จะตั้งฉากกัน (เวกเตอร์ลักษณะเฉพาะจากคนละค่า จุดกัน = 0 เสมอ)</p>
  </div>
  <div class="box box-def">
    <div class="box-title">📐 นิยาม — การแปลงเป็นทแยงมุมเชิงตั้งฉาก</div>
    <p>\(A\) <strong>แปลงเป็นทแยงมุมเชิงตั้งฉากได้</strong> (orthogonally diagonalizable) เมื่อมี \(P\) เชิงตั้งฉากปรกติ (หลัก orthonormal) และ \(D\) ทแยงมุมที่</p>
    \[ A = PDP^{-1} = PDP^T \]
  </div>
  <div class="box box-thm">
    <div class="box-title">⭐ ทฤษฎีบท 4.4.2 และ 4.4.3 — ทฤษฎีบทเชิงสเปกตรัม (Spectral Theorem)</div>
    <p><strong>\(A\) แปลงเป็นทแยงมุมเชิงตั้งฉากได้ ก็ต่อเมื่อ \(A\) สมมาตร</strong> — และสำหรับเมทริกซ์สมมาตร \(A\):</p>
    <p>1. ค่าลักษณะเฉพาะทุกตัวเป็น<strong>จำนวนจริง</strong></p>
    <p>2. \(\dim E_\lambda\) = จำนวนครั้งที่ \(\lambda\) ซ้ำเสมอ (ไม่มีปัญหา "eigenvector ไม่พอ" แบบ 3.2!)</p>
    <p>3. ปริภูมิลักษณะเฉพาะคนละค่าตั้งฉากกัน</p>
    <p>4. แปลงเป็นทแยงมุมเชิงตั้งฉากได้</p>
  </div>

  <h3>2) การแยกเชิงสเปกตรัม (spectral decomposition)</h3>
  <div class="box box-def">
    <div class="box-title">📐 สูตร</div>
    <p>เมื่อ \(A = PDP^T\) โดยหลักของ \(P\) คือเวกเตอร์หนึ่งหน่วย \(\vec{u}_1, \dots, \vec{u}_n\) กับค่าลักษณะเฉพาะ \(\lambda_1, \dots, \lambda_n\):</p>
    \[ A = \lambda_1\vec{u}_1\vec{u}_1^T + \lambda_2\vec{u}_2\vec{u}_2^T + \cdots + \lambda_n\vec{u}_n\vec{u}_n^T \]
    <p>แต่ละพจน์ \(\vec{u}_i\vec{u}_i^T\) เป็นเมทริกซ์จัตุรัสแรงก์ 1 ที่ "ฉายลงทิศ \(\vec{u}_i\)" — การแยกเชิงสเปกตรัมจึงเป็นการอธิบาย \(A\) ว่ายืดแต่ละทิศหลักเท่าไร</p>
  </div>

  <div class="box box-warn">
    <div class="box-title">⚠️ จุดต่างจาก 3.2 และจุดพลาดบ่อย</div>
    <p>• ใน 3.2 เราใช้ \(P\) ใดก็ได้ที่ไม่เอกฐาน แต่ในหัวข้อนี้ \(P\) ต้องเป็น<strong>เชิงตั้งฉากปรกติ</strong> (หลักยาว 1 + ตั้งฉากกัน) จึงจะได้ \(A = PDP^T\)</p>
    <p>• เวกเตอร์ในปริภูมิลักษณะเฉพาะเดียวกัน (ค่าซ้ำ) ไม่ได้ตั้งฉากกันโดยอัตโนมัติ — ต้อง<strong>กราม-ชมิดต์ภายในกลุ่มนั้น</strong> (ไม่ตัดข้ามกลุ่ม! เพราะกลุ่มคนละค่าตั้งฉากกันอยู่แล้ว)</p>
    <p>• ลำดับหลักของ \(P\) กับทแยงของ \(D\) ต้องตรงกัน และตรวจได้ด้วย \(P^TP = I\)</p>
  </div>
</section>
...
<section class="block" id="recipe">
  ...
  <div class="recipe">
    <div class="recipe-head">🪜 ท่าหลัก: แปลงสมมาตร → ทแยงมุมเชิงตั้งฉาก</div>
    <div class="recipe-body">
      <ol>
        <li>หาค่าลักษณะเฉพาะจาก \(\det(A - \lambda I) = 0\) (สมมาตร → จริงทั้งหมด)</li>
        <li>หาฐานหลักของแต่ละ \(E_\lambda\) จาก \((A - \lambda I)\vec{x} = \vec{0}\)</li>
        <li><strong>ถ้าค่าลักษณะเฉพาะซ้ำ:</strong> กราม-ชมิดต์<em>เฉพาะภายในกลุ่ม \(E_\lambda\) นั้น</em> (คนละกลุ่มตั้งฉากกันอยู่แล้ว)</li>
        <li>ปรับทุกเวกเตอร์ให้ยาว 1 → เรียงเป็นหลักของ \(P\), เรียง \(\lambda\) ตามลำดับเดียวกันลง \(D\)</li>
        <li>ตรวจ: \(P^TP = I_n\) และ \(A = PDP^T\) (หรือ \(A\vec{u}_i = \lambda_i\vec{u}_i\) ทุกตัว)</li>
        <li>สเปกตรัม: \(A = \sum_i \lambda_i\vec{u}_i\vec{u}_i^T\) — ตรวจได้โดยรวมเมทริกซ์แรงก์ 1 ทั้งหมด</li>
      </ol>
    </div>
  </div>
  <div class="key-grid">
    <div class="key-card"><div class="k-title">ท่า: จำสมบัติ 4 ข้อของสมมาตร</div>λ จริง / dim Eλ = จำนวนซ้ำ / คนละค่าตั้งฉาก / orthogonally diagonalizable เสมอ</div>
    <div class="key-card"><div class="k-title">ท่า: เช็กสมมาตรก่อนเสมอ</div>ถ้า \(A^T \ne A\) จะไม่มีทาง orthogonally diagonalizable ได้ (ทฤษฎีบท 4.4.2 ทิศย้อน)</div>
    <div class="key-card"><div class="k-title">ท่า: G-S เฉพาะในกลุ่ม</div>กราม-ชมิดต์ภายใน \(E_\lambda\) เดียวกันเท่านั้น — ห้ามผสมเวกเตอร์จากค่าลักษณะเฉพาะต่างกัน</div>
    <div class="key-card"><div class="k-title">ท่า: จำนวนคำตอบ</div>\(P\) เปลี่ยนเครื่องหมายหลัก/สลับหลักได้ (D ตาม) — ตรวจด้วย \(P^TP = I\) และ \(A\vec{u}_i = \lambda_i\vec{u}_i\)</div>
  </div>
</section>
...
<section class="block" id="practice">
  ...
  <article class="pr-card" data-pkey="p4-4-1">
    ...
    <div class="pr-body">
      <p>จงแปลง \(A = \begin{bmatrix} 8 & -2\\ -2 & 5 \end{bmatrix}\) เป็นทแยงมุมเชิงตั้งฉาก</p>
    </div>
  ...
  <article class="pr-card" data-pkey="p4-4-2">
    <div class="pr-head"><span class="pr-num">ข้อ 2</span><span class="diff">●●●</span><span class="spacer"></span>
      <label class="pr-done"><input type="checkbox"> ทำสำเร็จแล้ว</label></div>
    <div class="pr-body">
      <p>จงแปลง \(A = \begin{bmatrix} 2 & -1 & -1\\ -1 & 2 & -1\\ -1 & -1 & 2 \end{bmatrix}\) เป็นทแยงมุมเชิงตั้งฉาก (ทราบว่า \(\lambda = 0, 3, 3\))</p>
    </div>
  ...
  <article class="pr-card" data-pkey="p4-4-3">
    <div class="pr-head"><span class="pr-num">ข้อ 3</span><span class="diff">●●●</span><span class="spacer"></span>
      <label class="pr-done"><input type="checkbox"> ทำสำเร็จแล้ว</label></div>
    <div class="pr-body">
      <p>จงแปลง \(A = \begin{bmatrix} 25 & 5 & 10\\ 5 & 25 & -10\\ 10 & -10 & 10 \end{bmatrix}\) เป็นทแยงมุมเชิงตั้งฉาก (ทราบว่า \(\lambda = 30, 30, 0\))</p>
    </div>
  ...
  <article class="pr-card" data-pkey="p4-4-4">
    <div class="pr-head"><span class="pr-num">ข้อ 4</span><span class="diff">●●○</span><span class="spacer"></span>
      <label class="pr-done"><input type="checkbox"> ทำสำเร็จแล้ว</label></div>
    <div class="pr-body">
      <p>จงเขียนการแยกเชิงสเปกตรัมของ \(A = \begin{bmatrix} 5 & -2\\ -2 & 2 \end{bmatrix}\)</p>
    </div>
  ...
  <article class="pr-card" data-pkey="p4-4-5">
    <div class="pr-head"><span class="pr-num">ข้อ 5</span><span class="diff">●●○</span><span class="spacer"></span>
      <label class="pr-done"><input type="checkbox"> ทำสำเร็จแล้ว</label></div>
    <div class="pr-body">
      <p>จงแสดงว่าถ้า \(A\) เป็นเมทริกซ์สมมาตรแล้ว \((A\vec{v})\cdot\vec{w} = \vec{v}\cdot(A\vec{w})\) สำหรับเวกเตอร์ใด ๆ \(\vec{v}, \vec{w}\) แล้วตรวจยืนยันด้วย \(A = \begin{bmatrix} 1 & 2\\ 2 & 1 \end{bmatrix}\), \(\vec{v} = (1,0)^T\), \(\vec{w} = (0,1)^T\)</p>
    </div>
  ...
  <article class="pr-card" data-pkey="p4-4-6">
    <div class="pr-head"><span class="pr-num">ข้อ 6</span><span class="diff">●●●</span><span class="spacer"></span>
      <label class="pr-done"><input type="checkbox"> ทำสำเร็จแล้ว</label></div>
    <div class="pr-body">
      <p>จงแปลง \(A = \begin{bmatrix} 2 & 1 & 1\\ 1 & 2 & 1\\ 1 & 1 & 2 \end{bmatrix}\) เป็นทแยงมุมเชิงตั้งฉาก (ทราบว่า \(\lambda = 4, 1, 1\)) แล้วเขียนการแยกเชิงสเปกตรัมของ \(A\)</p>
    </div>
