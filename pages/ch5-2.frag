...
<section class="block" id="examples">
  ...
  <article class="ex-card">
    ...
    <div class="ex-body">
      ...
      <div class="approach"><span class="lbl">แนวคิด</span> — เอกพันธ์เชิงเส้นในสมาชิก → linear / กำลังสอง → หาตัวอย่างค้าน</div>
      <ol class="steps">
        <li><span class="step-t">(ก) เป็นการแปลงเชิงเส้น</span> ค่าออกมาเป็นผลรวมของ (สเกลาร์)×(สมาชิก) — สำหรับ \(A, B\) และ \(k\):
        \[ T(A + kB) = (a + kb') \dots = T(A) + kT(B) \;\checkmark \]
        (เขียนกระจายตามนิยามเมทริกซ์เสมอ — หรือสังเกตว่า \(T(A) = \vec{a}\cdot A\vec{?}\) ในรูปรวมเชิงเส้นของสมาชิก จึงเชิงเส้น)</li>
        <li><span class="step-t">(ข) ไม่เป็นการแปลงเชิงเส้น</span> หาตัวอย่างค้าน: \(A = \begin{bmatrix} 1 & 0\\ 0 & 0 \end{bmatrix}\): \(T(A) = 1\) แต่ \(2T(A) = 2\) ขณะที่ \(T(2A) = 2^2 + 0 = 4 \neq 2\) — สมบัติ \(T(kA) = kT(A)\) พัง ✗</li>
      </ol>
    </div>
  </article>
  ...
  <article class="ex-card">
    ...
    <div class="ex-body">
      ...
      <div class="approach"><span class="lbl">แนวคิด</span> — เขียน \(2 + 3x - x^2\) เป็นการรวมเชิงเส้นของ \(x+1, x-1, x^2\) (เทียบสัมประสิทธิ์) แล้วดึง \(T\) เข้าไป</div>
      <ol class="steps">
        <li><span class="step-t">แก้หาน้ำหนัก</span> \(2 + 3x - x^2 = \alpha(x+1) + \beta(x-1) + \gamma x^2\): เทียบสัมประสิทธิ์
        \[ \text{คงตัว: } \alpha - \beta = 2, \qquad x\text{: } \alpha + \beta = 3, \qquad x^2\text{: } \gamma = -1 \]
        บวกสองสมการแรก: \(2\alpha = 5 \Rightarrow \alpha = \tfrac52, \; \beta = \tfrac12, \; \gamma = -1\)</li>
        <li><span class="step-t">ดึง \(T\) เข้าไป</span>
        \[ T(2 + 3x - x^2) = \tfrac52\,T(x+1) + \tfrac12\,T(x-1) - T(x^2) = \tfrac52 x + \tfrac12 (1) - (-x^2) \]</li>
        <li><span class="step-t">สรุป</span>
        \[ T(2 + 3x - x^2) = \frac{1}{2} + \frac{5}{2}x + x^2 \]</li>
      </ol>
  ...
  <article class="ex-card">
    ...
    <div class="ex-body">
      ...
      <div class="approach"><span class="lbl">แนวคิด</span> — \(\ker T\) = พหุนามที่ราก 1 → เขียนเป็น Span / range = ค่าที่เป็นไปได้ทั้งหมด</div>
      <ol class="steps">
        <li><span class="step-t">หา \(\ker T\)</span> \(p = a + bx + cx^2\) อยู่ใน \(\ker T\) เมื่อ \(a + b + c = 0 \Rightarrow a = -b - c\):
        \[ p = b(x - 1) + c(x^2 - 1) \;\Longrightarrow\; \ker T = \operatorname{Span}\{x - 1,\; x^2 - 1\}, \;\; \operatorname{nullity} T = 2 \]</li>
        <li><span class="step-t">หา \(\operatorname{range} T\)</span> ค่า \(T(p) = p(1)\) เป็นจำนวนจริงใด ๆ เช่น \(T(k) = k\) สำหรับค่าคงตัว \(k\) → \(\operatorname{range} T = \mathbb{R}\), \(\operatorname{rank} T = 1\)</li>
        <li><span class="step-t">ตรวจทฤษฎีบทแรงก์</span> \(\dim \mathbb{R}_2[x] = 3 = \operatorname{rank} T + \operatorname{nullity} T = 1 + 2\) ✓ — และ \(T\) ทั่วถึง (rank = dim \(\mathbb{R}\) = 1) แต่ไม่ 1-1 (nullity &gt; 0)</li>
      </ol>
    </div>
  </article>
  ...
  <article class="ex-card">
    ...
    <div class="ex-body">
      ...
      <div class="approach"><span class="lbl">แนวคิด</span> — สมบัติอนุพันธ์ \((f + cg)' = f' + cg'\) → linear; เคอร์เนล = ฟังก์ชันที่อนุพันธ์เป็นศูนย์ = ค่าคงตัว</div>
      <ol class="steps">
        <li><span class="step-t">เป็นเชิงเส้น</span> จากกฎอนุพันธ์: \((f + cg)' = f' + cg'\) ✓ → \(D(f + cg) = D(f) + cD(g)\)</li>
        <li><span class="step-t">\(\ker D\)</span> \(f' = 0\) บน \((-\infty, \infty)\) ⇔ \(f\) เป็นฟังก์ชันค่าคงตัว:
        \[ \ker D = \{f : f(x) = c\} = \operatorname{Span}\{1\} \;\text{(มีมิติ 1)} \]</li>
        <li><span class="step-t">\(\operatorname{range} D\)</span> ฟังก์ชันใด ๆ \(g\) มีปฏิยานุพันธ์ \(f\) (เช่น \(f(x) = \int_0^x g(t)dt\)) ที่ \(D(f) = g\) → \(\operatorname{range} D = \mathbb{R}^{\mathbb{R}}\) → \(D\) ทั่วถึง แต่ไม่ 1-1 (เคอร์เนลไม่ชัด)</li>
      </ol>
    </div>
  </article>
</section>
...
<section class="block" id="practice">
  ...
  <article class="pr-card" data-pkey="p5-2-1">
    ...
    <details class="sol"><summary>แนวคิดและเฉลยแบบละเอียด</summary><div class="sol-body">
      <p><strong>แนวคิด:</strong> เช็กสมบัติการบวกและการคูณสเกลาร์</p>
      <ol class="steps">
        <li><span class="step-t">การบวก</span> \(\operatorname{tr}(A + B) = (a_{11} + b_{11}) + (a_{22} + b_{22}) = \operatorname{tr} A + \operatorname{tr} B\) ✓</li>
        <li><span class="step-t">สเกลาร์</span> \(\operatorname{tr}(kA) = ka_{11} + ka_{22} = k\operatorname{tr} A\) ✓</li>
        <li><span class="step-t">สรุป</span> ผ่านทั้งสองสมบัติ → <strong>\(T\) เป็นการแปลงเชิงเส้น</strong> (ฟังก์ชันนัลเชิงเส้น — linear functional)</li>
      </ol>
    </div></details>
  </article>
  ...
  <article class="pr-card" data-pkey="p5-2-2">
    ...
    <details class="sol"><summary>แนวคิดและเฉลยแบบละเอียด</summary><div class="sol-body">
      <p><strong>แนวคิด:</strong> การแปลงเชิงเส้นต้องส่ง \(\vec{0}\) ไป \(\vec{0}\)</p>
      <ol class="steps">
        <li><span class="step-t">แทนพหุนามศูนย์</span> \(T(0 + 0x + 0x^2) = 1 + x + x^2 \neq \vec{0}\) ✗</li>
        <li><span class="step-t">สรุป</span> \(T(\vec{0}_V) \neq \vec{0}_W\) → <strong>\(T\) ไม่เป็นการแปลงเชิงเส้น</strong> (เพราะมีการ "บวก 1" ทุกสัมประสิทธิ์ — จุดพังคลาสสิก)</li>
      </ol>
    </div></details>
  </article>
  ...
  <article class="pr-card" data-pkey="p5-2-3">
    ...
    <details class="sol"><summary>แนวคิดและเฉลยแบบละเอียด</summary><div class="sol-body">
      <p><strong>แนวคิด:</strong> เขียนเป็นรวมเชิงเส้นของเวกเตอร์ที่รู้ค่า แล้วดึง \(T\) เข้าไป</p>
      <ol class="steps">
        <li><span class="step-t">แก้หาน้ำหนัก</span> เทียบสัมประสิทธิ์: คงตัว \(2\alpha + \beta = -1\); \(x\): \(-\alpha + \beta = 2\) ลบกัน: \(3\alpha = -3 \Rightarrow \alpha = -1, \; \beta = 1\)
        \[ -1 + 2x = -(2 - x) + (1 + x) \]</li>
        <li><span class="step-t">ดึง \(T\) เข้าไป</span>
        \[ T(-1 + 2x) = -T(2 - x) + T(1 + x) = -(1, -1, 1)^T + (0, 1, 0)^T = (-1, 2, -1)^T \]</li>
        <li><span class="step-t">ตรวจคำตอบ</span> น้ำหนักย้อน: \(-(2-x) + (1+x) = -1 + 2x\) ✓</li>
      </ol>
    </div></details>
  </article>
  ...
  <article class="pr-card" data-pkey="p5-2-4">
    ...
    <details class="sol"><summary>แนวคิดและเฉลยแบบละเอียด</summary><div class="sol-body">
      <p><strong>แนวคิด:</strong> แปลเงื่อนไข \(T(A) = \vec{0}\) เป็นสมบัติของ \(A\)</p>
      <ol class="steps">
        <li><span class="step-t">\(\ker T\)</span> \(T(A) = 0 \Leftrightarrow A = A^T\) → เซตของเมทริกซ์สมมาตร
        ...
        <li><span class="step-t">\(\operatorname{range} T\)</span> สังเกต \(T(A)^T = A^T - A = -T(A)\) → ผลลัพธ์เป็นเมทริกซ์แอนติสมมาตร \(\begin{bmatrix} 0 & x\\ -x & 0 \end{bmatrix}\) และทุกตัวแบบนี้เกิดจาก \(T\left(\begin{bmatrix} 0 & x/2\\ -x/2 & 0 \end{bmatrix}\right)\) →
        \[ \operatorname{range} T = \operatorname{Span}\left\{ \begin{bmatrix} 0 & 1\\ -1 & 0 \end{bmatrix} \right\}, \;\; \operatorname{rank} T = 1 \]</li>
        <li><span class="step-t">ตรวจทฤษฎีบทแรงก์</span> \(\dim M_2(\mathbb{R}) = 4 = 1 + 3\) ✓</li>
      </ol>
    </div></details>
  </article>
  ...
  <article class="pr-card" data-pkey="p5-2-5">
    ...
    <details class="sol"><summary>แนวคิดและเฉลยแบบละเอียด</summary><div class="sol-body">
      <p><strong>แนวคิด:</strong> แปลงเงื่อนไขการแทนค่าเป็นระบบสมการสัมประสิทธิ์</p>
      <ol class="steps">
        <li><span class="step-t">เชิงเส้น</span> \(T(p + cq) = ((p+cq)(0), (p+cq)(1)) = (p(0) + cq(0), p(1) + cq(1)) = T(p) + cT(q)\) ✓</li>
        <li><span class="step-t">\(\ker T\)</span> \(p = a_0 + a_1x + a_2x^2\): \(p(0) = a_0 = 0\) และ \(p(1) = a_0 + a_1 + a_2 = 0 \Rightarrow a_1 = -a_2\):
        \[ p = a_2(x^2 - x) \;\Longrightarrow\; \ker T = \operatorname{Span}\{x^2 - x\}, \;\; \operatorname{nullity} T = 1 \]</li>
        <li><span class="step-t">ทั่วถึงหรือไม่</span> \(\operatorname{rank} T = 3 - 1 = 2 = \dim \mathbb{R}^2\) → <strong>ทั่วถึง</strong> (ตัวอย่าง: \(T(x) = (0,1)^T\), \(T(1-x) = (1,0)^T\)) — แต่ไม่ 1-1 เพราะเคอร์เนลไม่ชัด</li>
      </ol>
    </div></details>
  </article>
  ...
  <article class="pr-card" data-pkey="p5-2-6">
    ...
    <details class="sol"><summary>แนวคิดและเฉลยแบบละเอียด</summary><div class="sol-body">
      <p><strong>แนวคิด:</strong> อนุพันธ์ลดดีกรีลง 1 → พจน์คงตัวหาย</p>
      <ol class="steps">
        <li><span class="step-t">\(\ker T\)</span> \(T(a_0 + a_1x + a_2x^2) = a_1 + 2a_2x = \vec{0} \Rightarrow a_1 = a_2 = 0\):
        \[ \ker T = \operatorname{Span}\{1\} \;\text{(ค่าคงตัว)}, \;\; \operatorname{nullity} T = 1 \;\Longrightarrow\; T \text{ ไม่ 1-1} \]</li>
        <li><span class="step-t">\(\operatorname{range} T\)</span> ผลลัพธ์เป็น \(a_1 + 2a_2x\) ซึ่งครอบคลุมพหุนามดีกรี ≤ 1 ทุกตัว:
        \[ \operatorname{range} T = \operatorname{Span}\{1, x\} = \mathbb{R}_1[x], \;\; \operatorname{rank} T = 2 \;\Longrightarrow\; T \text{ ไม่ทั่วถึง } \mathbb{R}_2[x] \]</li>
        <li><span class="step-t">ตรวจทฤษฎีบทแรงก์</span> \(3 = 2 + 1\) ✓ (และสอดคล้อง: \(T: V \to V\) มิติเท่ากัน แต่ไม่ 1-1 ก็ไม่ทั่วถึง — เหมือนข้อสรุปใน 2.1)</li>
      </ol>
    </div></details>
  </article>
</section>
